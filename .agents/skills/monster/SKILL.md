...
2. **Icons/Assets**: If adding monster icons, ensure an **8-bit medium granularity pixel art style with a completely transparent background (fully de-backgrounded to remove all white halos/edges) and thick outlines** (similar to classic retro RPG battle sprites). Generate using `generate_image` and save as PNG. To prevent white edges and residual white blocks, you **must** use the custom high-precision background removal script stored in [.agents/scripts/remove_background.py](file:///c:/Users/Ted/WebstormProjects/tower/.agents/scripts/remove_background.py). Run it using: `python .agents/scripts/remove_background.py <input_path> <output_path>`. This script uses connected component BFS to preserve small armor highlights (size <= 15) while completely clearing the background and running 2 passes of edge halo-clearing (removing R,G,B >= 160 next to alpha=0).
